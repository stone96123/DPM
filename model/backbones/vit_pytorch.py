...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        mask = self.ConvLayer(mask)
        mask = self.attfc(torch.squeeze(mask))
        x = self.norm(x)
        return x[:, 0], mask, orth_proto

    def forward(self, x, cam_label=None, view_label=None):
        x, mask, orth_proto = self.forward_features(x, cam_label, view_label)
        return x, self.sigmoid(mask), orth_proto
