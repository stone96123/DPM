...
class MArcface(nn.Module):
    ...
    def forward(self, input, mask, label):
        ...
        for i in range(input.shape[0]):
            if i == 0:
                query_vector = F.normalize(input[i,:].unsqueeze(0))
                gallery_prototype = mask[i,:].unsqueeze(0) * F.normalize(self.weight)
                cosine = F.linear(F.normalize(query_vector), F.normalize(gallery_prototype))
            else:
                query_vector = F.normalize(input[i,:].unsqueeze(0))
                gallery_prototype = mask[i,:].unsqueeze(0) * F.normalize(self.weight)
                cosine_single = F.linear(F.normalize(query_vector), F.normalize(gallery_prototype))
                cosine = torch.cat((cosine, cosine_single), 0)
        sine = torch.sqrt(1.0 - torch.pow(cosine, 2))
        phi = cosine * self.cos_m - sine * self.sin_m
        phi = phi.type_as(cosine)
        if self.easy_margin:
